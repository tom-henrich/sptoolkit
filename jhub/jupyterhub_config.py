...
"""JupyterHub config for the jhub jupyterhub.
...
from jhub import configurer, user
from jhub.config import INSTALL_PREFIX, USER_ENV_PREFIX, CONFIG_DIR
from jhub.normalize import generate_system_username
from jhub.utils import get_plugin_manager
...
class UserCreatingSpawner(SystemdSpawner):
    ...
    """
    user_groups = Dict(key_trait=Unicode(),
...
# Use high port so users can try this on machines with pre-existing JupyterHub
...
jhub_config = configurer.load_config()
configurer.apply_config(jhub_config, c)

# Let hooks modify `c` if they want

# Call custom configuration plugin
pm = get_plugin_manager()
pm.hook.jhub_custom_jupyterhub_config(c=c)

# Load arbitrary .py config files if they exist.
# This is our escape hatch
extra_configs = sorted(glob(os.path.join(CONFIG_DIR,
                                         'jupyterhub_config.d',
                                         '*.py')))
