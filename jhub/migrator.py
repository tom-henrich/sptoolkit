...
"""Migration utilities for upgrading jhub.
...
def migrate_file(old_path, new_path):
    ...
    This avoids collisions if the new file exists.
    ...
    if os.path.exists(new_path):
        ...
        logger.warning(f"Moving {old_path} to {dest} to avoid clobbering.  Its contents will be ignored.")
    else:
        dest = new_path
...
def migrate_config_files():
    ...
    """
    # handle old JHUB/config.yaml location
