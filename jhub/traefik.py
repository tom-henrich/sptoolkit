...
from jhub.configurer import load_config, _merge_dictionaries
...
checksums = {"linux-amd64": "3c2d153d80890b6fc8875af9f8ced32c4d684e1eb5a46d9815337cb343dfd92e"}
...
def fatal_error(e):
    # Retry only when connection is reset or
    # we think we didn't download entire file
    return str(e) != "ContentTooShort" and not isinstance(e,
...
def ensure_traefik_config(state_dir):
    ...
    if https["enabled"]:
        if not tls["cert"] and not letsencrypt["email"]:
            raise ValueError("To enable https, you must set tls.cert+key or letsencrypt.email+domains")
    ...
    try:
        # Load standard config file and merge it with the extra config files
