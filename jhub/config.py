...
Note:
    Commands include
    - jhub-config set firstlevel.second_level something

    - jhub-config show

    - jhub-config show firstlevel

    - jhub-config show firstlevel.second_level
...
def set_item_in_config(config, property_path, value):
    """Set key at property_path with config value and return new config.

    The config itself is not mutated.

    property_path is a series of dot separated values. Any part of the
    path that does not exist is created.
    ...
    for i, cur_path in enumerate(path_components):
        ...
        if i == len(path_components) - 1:
            ...
            # If asked to create new non-leaf nodes, always make them
            # dicts. This means that setting a value is *destructive*,
            # meaning it will replace whatever is down there!
...
def check_hub_ready():
    """Checks if JupyterHub is ready.

    """
    from .configurer import load_config
    http_port = load_config()['http']['port']
    try:
        r = requests.get('http://127.0.0.1:%d/hub/api' % http_port,
                         verify=False)
...
def reload_component(component):
    """Reloads the jhub component specified.
    ...
    from jhub import systemd, traefik
...
def parse_value(value_str):
    """Parses a given string value into a specific type.
...
def main(argv=None):
    """Executes the config commandline argv if valid.

    """
    if os.geteuid() != 0:
        print("It looks like this command wasn't run with root privileges."
              "Did you use the command: `sudo -E`?")
    ...
    try:
        ...
        print(str(e))
        print("Did you use the command: `sudo -E`?")

    argparser = argparse.ArgumentParser()
    argparser.add_argument(
        '--config-path',
        default=CONFIG_FILE,
        help='The path to jhub config.yaml file.'
    )
    ...
    unset_parser.add_argument(
        'key_path',
        help='Dot separated path to the configuration key to unset.'
    )
    ...
    set_parser.add_argument(
        'value',
        help='Value to set the specified configuration key to.'
    )
    ...
    reload_parser.add_argument(
        'component',
        choices=('hub', 'proxy'),
        help='Which component you would like to reload.',
        default='hub',
        nargs='?'
    )
    ...
    if args.action == 'show':
        ...
        set_config_value(args.config_path,
                         args.key_path,
                         parse_value(args.value))
    elif args.action == 'unset':
        unset_config_value(args.config_path,
                           args.key_path)
    elif args.action == 'add-item':
        add_config_value(args.config_path,
                         args.key_path,
                         parse_value(args.value))
    elif args.action == 'remove-item':
        remove_config_value(args.config_path,
                            args.key_path,
                            parse_value(args.value))
