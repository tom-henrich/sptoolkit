...
"""Installation logic for JupyterHub.
...
def remove_chp():
    """Ensure configurable-http-proxy is not running.
...
def ensure_jupyterlab_extensions():
    ...
    """
    extensions = [
        # Don't pin versions here, since labextension will find
        # something appropriate for our version of jupyterlab
        '@jupyter-widgets/jupyterlab-manager'
    ]
    # do not build extension at install time. Will build later
    install_options = [
        '--no-build'
    ]
...
def ensure_jupyterhub_package(prefix):
    ...
    hub environment. A lot of spawners & authenticators do not have
    conda-forge packages, but do have pip packages. Keeping all python
    packages in the hub environment be installed with pip prevents
    accidental mixing of python and conda packages!

    """
    # JupyterHub prefers pycurl over SimpleHTTPClient automatically
    # pycurl is generally more bugfree. build-essential is also
    # generally useful to everyone involved, and required for pycurl
...
def ensure_usergroups():
    """Sets up user groups and their sudo rules.

    `sudo -E` should preserve the $PATH we set. This allows admins in
    jupyter terminals to do `sudo -E pip install <package>`. `pip` is in
    the $PATH set in jupyterhub_config.py to include the user conda env.
...
def ensure_user_environment(user_requirements_txt_file):
    ...
    if conda.check_miniconda_version(USER_ENV_PREFIX, miniconda_new_version):
        ...
        conda_version = '4.5.8'
    # If no prior miniconda installation is found, install a newer version
    else:
        logger.info('Downloading and setting up user environment...')
    ...
    if user_requirements_txt_file:
        # FIXME: currently fails hard, should fail soft and not abort installer
...
def ensure_jupyterhub_running(times=20):
    ...
    for i in range(times):
        try:
            logger.info('Waiting for JupyterHub to start({}/{} tries)'.format(i + 1, times))
            # Does not matter that SSL is valid at this level so
            # suppress InsecureRequestWarning for this request.
...
def ensure_symlinks(prefix):
    ...
    but not the hub venv. This means jhub-config is not actually
    accessible.
    ...
    """
    jhub_config_src = os.path.join(prefix, 'bin', 'jhub-config')
    jhub_config_dest = '/usr/bin/jhub-config'
    if os.path.exists(jhub_config_dest):
        if os.path.realpath(jhub_config_dest) != jhub_config_src:
            # jhub-config exists that isn't ours. We should *not* delete this
            # file, instead we throw an error and abort. Deleting files owned
            # by other people while running as root is dangerous, especially
            # with symlinks involved.
            raise FileExistsError(f'/usr/bin/jhub-config exists but is not a symlink to {jhub_config_src}')
        else:
            # We have a working symlink, so do nothing
            return
    os.symlink(jhub_config_src, jhub_config_dest)
...
def run_plugin_actions(plugin_manager):
    ...
    apt_packages = list(set(itertools.chain(*hook.jhub_extra_apt_packages())))
    if apt_packages:
    ...
    hub_pip_packages = list(set(itertools.chain(*hook.jhub_extra_hub_pip_packages())))
    if hub_pip_packages:
    ...
    conda_packages = list(set(itertools.chain(*hook.jhub_extra_user_conda_packages())))
    if conda_packages:
    ...
    user_pip_packages = list(set(itertools.chain(*hook.jhub_extra_user_pip_packages())))
...
def ensure_config_yaml(plugin_manager):
    ...
    hook = plugin_manager.hook
    hook.jhub_config_post_install(config=config)
...
def main():
    ...
    argparser.add_argument(
        ...
        help='URL to a requirements.txt file to be installed in the user environment'
    )
    argparser.add_argument(
        '--plugin',
        nargs='*',
